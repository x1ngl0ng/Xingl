...
# excel 以下——————————————————————————————————————————————————————————————————————————————————————————————————————
    def login_success_data(self,filename,sheet_name=None):#1个用于读取excel文件的函数，参数代表：要读取的sheet页，默认None
        data = []
...
#以下db——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    def execute_query(self,host, user, password, database, query, params=None, charset = 'utf8mb4', port = '3306'):
...
if __name__ == '__main__':
    ...
    results = execute_query(host, user, password, database, query, params)
    for row in results:
        print(row)
    #----------------------------------------------------------------------------------------------
